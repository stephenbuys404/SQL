-- creating a gis_analysis database
CREATE DATABASE gis_analysis;

-- loading the PostGIS extension
CREATE EXTENSION postgis;

SELECT postgis_full_version(); -- shows PostGIS version

-- retrieving the well-known text for SRID 4326
SELECT srtext
...
SELECT ST_GeomFromText('MULTIPOLYGON(((-74.92 42.7, -75.06 42.71,-75.07 42.64, -74.92 42.7),(-75.0 42.66, -75.0 42.64,
...
SELECT ST_MPolyFromText('MULTIPOLYGON(((-74.92 42.7, -75.06 42.71,-75.07 42.64, -74.92 42.7),(-75.0 42.66, -75.0 42.64,
                                        -74.98 42.64, -74.98 42.66,
                                        -75.0 42.66)
                                      ))', 4326);

-- create and load the farmers_markets table
...
FROM 'C:\edb\farmers_markets.csv'
...
-- locate farmers' markets within 10 kilometers of a point
SELECT market_name,
       city,
       st
FROM farmers_markets
